...
try {
  ...
  // });
  // const responseScan = await ddbDocClient.send(scanCmd);
  // console.log(`ScanCommand response: ${JSON.stringify(responseScan, null, 2)}`);
  ...
  const responseBatch = await ddbDocClient.send(batchGetCmd);
  console.log(`BatchGetCommand response: ${JSON.stringify(responseBatch, null, 2)}`);
} catch (error) {
